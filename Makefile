...
# =====================================================================================
# ondewo-s2t-client-go - Makefile
#
# The ONDEWO S2T (Speech-to-Text) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-s2t-api submodule by the
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-s2t-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_S2T_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_S2T_VERSION}
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO S2T Go Client ${ONDEWO_S2T_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-s2t-client-go"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-s2t-client-go repo
	pre-commit install
...
print_version: ## Print ONDEWO_S2T_VERSION and nothing else
	@echo "${ONDEWO_S2T_VERSION}"
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-s2t-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-s2t-client-go@v7.5.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_S2T_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_S2T_VERSION=${ONDEWO_S2T_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_S2T_VERSION=${ONDEWO_S2T_VERSION}"
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO S2T Go Client ${ONDEWO_S2T_VERSION}' entry"; \
